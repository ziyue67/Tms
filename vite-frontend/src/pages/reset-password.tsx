...
export default function ResetPasswordPage() {
  ...
  return <DefaultLayout><section className="flex min-w-0 justify-center py-8"><Card className="w-full min-w-0 max-w-md"><CardHeader className="flex-col items-center"><h1 className={title({ size: "sm" })}>设置新密码</h1><p className="text-small text-default-500 mt-2">重置链接只能使用一次</p></CardHeader><CardBody className="gap-4"><Input label="新密码" type="password" value={password} onChange={(e) => setPassword(e.target.value)} /><Input label="确认新密码" type="password" value={confirm} onChange={(e) => setConfirm(e.target.value)} /><Button color="primary" size="lg" onClick={submit} isLoading={loading}>重置密码</Button><div className="text-center text-sm text-default-500"><Link className="text-primary" to="/">返回登录</Link></div></CardBody></Card></section></DefaultLayout>;
}

...
export default function RegisterPage() {
  ...
  return <DefaultLayout><section className="flex min-w-0 justify-center py-8"><Card className="w-full min-w-0 max-w-md"><CardHeader className="flex-col items-center"><h1 className={title({ size: "sm" })}>注册账号</h1><p className="text-small text-default-500 mt-2">创建 TMS 账号</p></CardHeader><CardBody className="gap-4">{!authConfig.registrationEnabled ? <p className="text-center text-sm text-danger">管理员已关闭新用户注册</p> : <><Input label="邮箱" type="email" value={form.email} onChange={(e) => change("email", e.target.value)} /><Input label="用户名（可选）" value={form.username} onChange={(e) => change("username", e.target.value)} /><Input label="密码" type="password" value={form.password} onChange={(e) => change("password", e.target.value)} />{authConfig.emailVerificationEnabled && <div className="flex flex-col gap-2 sm:flex-row"><Input className="min-w-0 flex-1" label="邮箱验证码" value={form.code} onChange={(e) => change("code", e.target.value)} /><Button className="w-full shrink-0 sm:mt-2 sm:w-auto" variant="flat" onClick={send} isLoading={sending} isDisabled={cooldown > 0}>{cooldown > 0 ? `${cooldown}s` : "获取验证码"}</Button></div>}<Button color="primary" size="lg" onClick={submit} isLoading={loading}>注册</Button></>}<div className="text-center text-sm text-default-500">已有账号？<Link className="text-primary ml-1" to="/">返回登录</Link></div></CardBody></Card></section></DefaultLayout>;
}

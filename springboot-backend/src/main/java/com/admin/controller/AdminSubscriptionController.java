...
public class AdminSubscriptionController {
    ...
    @RequireRole @PostMapping("/plans/{id}/disable") public R disable(@PathVariable long id) { SubscriptionPlan p = plans.selectById(id); if (p == null) return R.err("套餐不存在"); p.setStatus(0); p.setUpdatedTime(System.currentTimeMillis()); plans.updateById(p); return R.ok(); }
    @Transactional
    @RequireRole @DeleteMapping("/plans/{id}") public R delete(@PathVariable long id) { try { SubscriptionPlan p = plans.selectById(id); if (p == null) return R.err("套餐不存在"); plans.deleteById(id); return R.ok(); } catch (RuntimeException e) { return R.err("删除套餐失败: " + rootMessage(e)); } }

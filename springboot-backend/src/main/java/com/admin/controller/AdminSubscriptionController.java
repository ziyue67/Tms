...
public class AdminSubscriptionController {
    ...
    @RequireRole @PutMapping("/plans/{id}") public R update(@PathVariable long id, @RequestBody SubscriptionPlan plan) { try { SubscriptionPlan existing = plans.selectById(id); if (existing == null) return R.err("套餐不存在"); mergeMissing(plan, existing); validate(plan); plan.setId(id); plan.setUpdatedTime(System.currentTimeMillis()); if (plans.updateById(plan) != 1) return R.err("更新套餐失败"); return R.ok(plans.selectById(id)); } catch (IllegalArgumentException e) { return R.err(e.getMessage()); } catch (RuntimeException e) { return R.err("更新套餐失败: " + rootMessage(e)); } }
    @RequireRole @PostMapping("/plans/{id}/disable") public R disable(@PathVariable long id) { try { SubscriptionPlan p = plans.selectById(id); if (p == null) return R.err("套餐不存在"); p.setStatus(0); p.setForSale(0); p.setRedeemable(0); p.setUpdatedTime(System.currentTimeMillis()); if (plans.updateById(p) != 1) return R.err("停用套餐失败"); return R.ok(plans.selectById(id)); } catch (RuntimeException e) { return R.err("停用套餐失败: " + rootMessage(e)); } }
    @Transactional
    ...
    @Transactional
    @RequireRole @DeleteMapping("/redeem-codes/{id}") public R deleteRedeemCode(@PathVariable long id) { try { service.deleteRedeemCode(id); return R.ok(); } catch (IllegalArgumentException e) { return R.err(e.getMessage()); } }

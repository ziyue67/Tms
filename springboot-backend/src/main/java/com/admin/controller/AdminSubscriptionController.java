...
public class AdminSubscriptionController {
    ...
    @RequireRole @PostMapping("/plans") public R create(@RequestBody SubscriptionPlan plan) { try { validate(plan); plan.setStatus(plan.getStatus() == null ? 1 : plan.getStatus()); plan.setResetQuota(plan.getResetQuota() == null ? 1 : plan.getResetQuota()); plan.setCreatedTime(System.currentTimeMillis()); plan.setUpdatedTime(System.currentTimeMillis()); plans.insert(plan); return R.ok(plan); } catch (IllegalArgumentException e) { return R.err(e.getMessage()); } }
    @RequireRole @PutMapping("/plans/{id}") public R update(@PathVariable long id, @RequestBody SubscriptionPlan plan) { try { SubscriptionPlan existing = plans.selectById(id); if (existing == null) return R.err("套餐不存在"); validate(plan); plan.setId(id); plan.setResetQuota(plan.getResetQuota() == null ? (existing.getResetQuota() == null ? 1 : existing.getResetQuota()) : plan.getResetQuota()); plan.setUpdatedTime(System.currentTimeMillis()); plans.updateById(plan); return R.ok(); } catch (IllegalArgumentException e) { return R.err(e.getMessage()); } }
